...
async def lifespan(app: FastAPI):
    ...
    # SKIP_LIFESPAN_ALEMBIC=1 でスキップ可能（ローカルSQLite開発用。本番Vercelでは未設定なので通常どおり実行）
    if os.getenv("SKIP_LIFESPAN_ALEMBIC") == "1":
        logger.info("Alembic upgrade skipped (SKIP_LIFESPAN_ALEMBIC=1)")
    else:
        try:
