...
NBR_IVT_DETECTED = 506          # IVT（無効トラフィック）を検出したためノービッド（#8）
NBR_BRAND_SAFETY_BLOCK = 507    # ブランドセーフティ（bcat/badv）でブロックされたためノービッド（#8）
...
NBR_LABELS: dict[int, str] = {
    ...
    NBR_IVT_DETECTED: "IVT (invalid traffic) detected",
    NBR_BRAND_SAFETY_BLOCK: "Brand safety block (bcat/badv)",
}
