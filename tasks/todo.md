...
### Phase 2.6 — アドテク技術レビュー反映（2026-05-22）

別エージェント（アドテク専門レビュー）が DSP/SSP 技術スキルマップ9領域で dsp_engine を監査し、
20件の改善点を優先度付きで報告。うち High 中心の5件を修正。

- [x] エクスチェンジ認証: DspConfigDB に api_secret 追加、inbound_bid で X-DSP-Secret 照合（マイグレーション dspengine0003）
- [x] 総予算制御: pacing.can_bid が total_budget_jpy（累計消化）もチェック。SSP連携画面に認証シークレット入力欄追加
- [x] 入札パス N+1 解消: get_all_campaign_stats（一括IN集計、3クエリ）で 3N+1 → 4 クエリに削減
- [x] 落札の冪等化: record_dsp_win が同一 click_token を二重計上しない（nurl再送対策）
- [x] 配信期間: list_active_campaigns が start_date/end_date でフィルタ（期間外は入札対象外）
- [x] テスト7件追加（計38 dsp_engineテスト green）。全体 190 passed・回帰なし・スモーク全PASS。

未対応（次バッチ候補・レビュー指摘 #4,#7-20）:
- can_bid→record_spend の TOCTOU レース（完全対応は Redis Lua。現状は安全率90%で緩和）
- nbr（no-bid理由コード）未設定、クリック連打レート制限、raw_payload の PII サニタイズ
- (campaign_id, logged_at) 複合インデックス、device 特徴量の入札反映、複数クリエイティブ/A・Bテスト
- sellers.json/ads.txt エンドポイント、bid shading

### #4 入札ログ完全化 + 予算 TOCTOU 対策（2026-05-22）

handoff #4・#8 / progress 優先#4。TDD（Red 先行）で実装。

- [x] `dsp_engine/nbr.py` 新規 — no-bid 理由コード（標準 0/1/2 + 拡張 500-504）+ ラベル表
- [x] `DspBidLogDB`（dsp_bid_logs）追加 — bid request 全件を nbr 付きで記録
- [x] マイグレーション dspengine0005 — dsp_bid_logs テーブル（冪等・has_table ガード）
- [x] `handle_bid_request` 全分岐に `_log_bid_decision`（DB 全行 + Redis nbr 集計カウンタ）
- [x] `record_dsp_win` に `_maybe_exhaust_budget` — 総予算超過で status=budget_exhausted 自動切替（TOCTOU 抑止）
- [x] `pacing.record_spend` の INCRBYFLOAT+EXPIRE を Lua スクリプトで原子化
- [x] admin エンドポイント `GET /dsp-engine/admin/bid-logs/api`（直近ログ + nbr 内訳 JSON）
- [x] テスト `tests/test_dsp_bid_log.py` 12件追加・全 PASS。全体 258 passed（6 failed は既知 MDM 事前不具合）
- [x] マイグレーション検証: populated DB コピー + stamp dspengine0004 + upgrade head で 0005 適用確認（教訓16）

レビュー:
- nbr は OpenRTB 標準コード（スパイダー等のエクスチェンジ視点）が DSP の no-bid 理由を表現できないため、500番台の exchange-specific 領域に独自コードを定義。
- TOCTOU は bid≠win の構造上「ゼロオーバーラン」は不可能。win 確定直後に実消化（DspSpendLogDB 合計）で再判定し、超過時にキャンペーンを確定停止することで被害を in-flight 分に限定する設計とした。
- ログ書き込み失敗が入札を巻き込まないよう例外は握りつぶすが、silent failure を避けるため warning ログは必ず出力。
- 残: 本番デプロイ（vercel --prod）+ dspengine0003-0005 適用、bid log のフル admin UI（#6）、入札パス DB write の batching（#10）。

### #5 pCTR/pCVR/value/win-rate ベースライン ML（2026-05-22）

handoff #7・#13 / progress 優先#5。TDD（Red 先行）で実装。手法は統計ベースライン（shrinkage）。

- [x] `config.py` に `warm_threshold` 追加（旧 WARM_THRESHOLD ハードコードを設定化）
- [x] `scoring.py` 刷新 — pCTR×pCVR×value を経験ベイズ shrinkage 推定（`_shrink` / `predict_ctr` / `predict_cvr` / `predict_value`）。実績 50 件硬切替の cliff を廃止
- [x] `DspSegmentPerfDB`（dsp_segment_perf）+ マイグレーション dspengine0006
- [x] `segments.py` 新規 — `platform_of` / L1 キャッシュ / `recompute_segment_multipliers`（platform 別 CTR 乗数バッチ・[0.5,2.0] クランプ）/ `schedule_segment_tasks`
- [x] `bidder.py` — handle_bid_request が device セグメント乗数を pCTR に反映（`compute_bid_cpm_jpy(ctr_multiplier=)`）
- [x] `bidder.py` `get_campaign_win_rates` 追加 — win-rate 算出。admin/bid-logs/api に `campaign_win_rates` 追加
- [x] `main.py` lifespan に segment バッチタスク登録
- [x] テスト `tests/test_ml_scoring.py` 13件追加・全 PASS。全体 271 passed（6 failed は既知 MDM 事前不具合）
- [x] 既存 scoring/shading テスト 3件を shrinkage モデルに合わせ更新（`test_scoring_warm_blends_observed_with_prior` / shading 2件は期待値をモデル算出ベースに）
- [x] マイグレーション検証: populated DB コピー + stamp dspengine0004 + upgrade head で 0005・0006 適用確認（教訓16）

レビュー:
- 「ベースライン ML」を統計手法（経験ベイズ shrinkage）で実装。TensorFlow 不要・純関数で入札パスに安全。two_tower（MDM デバイス推薦用・TF 依存）は DSP キャンペーン入札と設計が別物のため接続せず。
- shrinkage 化で「実績を seed したが click が無い」テスト campaign の入札が下がった（観測 CTR=0% を正しく学習）。これは cliff 廃止の正しい帰結。期待値ハードコードのテストはモデル算出ベースに是正。
- device セグメントは現状 spend/click ログが持つ `platform` 粒度。devicetype/os/geo の細分化は spend/click ログへの記録追加が前提のため別タスク。
- win-rate は可視化のみ（入札非反映）。落札率連動入札は将来タスク。
- 残: 本番デプロイ（vercel --prod）+ dspengine0003-0006 適用、#6 多次元レポート拡張。

### #3 サプライチェーン検証レビュー指摘 3 件の修正（2026-05-22）

優先 #3 のレビューで判明した実バグ・脆弱性 3 件を TDD（Red 先行）で修正。

- [x] 指摘1 High: `inbound_bid` が接続名（任意文字列）を asi として `verify_schain` に渡し正当な schain を全誤拒否 → `dsp_configs.exchange_asi` カラム追加（migration `dspengine0007`）、`verify_schain` に渡す値を `exch.exchange_asi` に変更、未設定時は最終ノード検証スキップ、SSP 連携画面に asi 入力欄追加
- [x] 指摘2 High: sellers.json 突合が全 schain ノードを 1 社の sellers.json で検証し多段 schain を誤 no-bid → `verifiable_nodes()` 追加し asi 一致ノードのみ突合
- [x] 指摘3 Medium: win notice(nurl) が無署名で第三者が spend 偽装可能 → `sign_win_notice`/`verify_win_notice`（HMAC-SHA256）追加、`/dsp-engine/win` で署名必須化（不正は 403）、price 改竄対策に `cleared_usd` を bid で上限クランプ
- [x] テスト: test_supply_chain 6件 / test_dsp_engine 6件 追加。全体 293 passed（6 failed は既知 MDM 事前不具合）
- [x] マイグレーション検証: populated DB コピー + stamp dspengine0006 + `upgrade dspengine0007` + 冪等性確認（教訓16/17）

レビュー:
- exchange_asi 未設定（既存接続）は後方互換: 最終ノード検証・sellers.json 突合をスキップし従来どおり通す。SSP 連携画面に asi 欄を新設したので新規接続では設定可能。
- win notice の price は `${AUCTION_PRICE}` マクロのため署名対象に含められない。署名で cid 偽造を遮断し、再生は `record_dsp_win` の click_token 冪等で二重計上を防ぎ、price 改竄は bid 上限クランプで緩和。
- migration revision は handoff の記述（0001〜0006）でなく `alembic heads` 実測で採番（教訓17）。`dspengine0008` は別タスク #6 の未コミット WIP のため不可侵。
- 残: 本番デプロイ（vercel --prod）+ dspengine0007 適用は Fuji さん許可後。コミットも未実施（指示待ち）。
