...
### 23. 並行セッションの lessons/docs 記述は自他の作業を取り違えうる（git log で一次検証）
- Date: 2026-05-23（2026-05-23 訂正）/ Trigger: 本エントリ旧版は commit 842683b/f1f6589/d998006 を「別セッションの重複作業」と記述したが、実際はそれらは本セッションが Fujiさん承認のうえ test-first-implement で実施したセキュリティ修正3件の正規 commit で、重複は発生していない。並行セッションが他セッションの commit を誤読して誤った lessons を書いていた。
- Root Cause: 並行セッションは他セッションの作業状況を直接知れず、git log だけで commit の帰属を推測するため誤認しやすい。lessons/handoff/docs は二次情報で誤りが混入しうる。
- Mitigation: 着手前に `git log --oneline origin/master..master` で進行中作業を確認するのは有効。ただし他セッションが書いた lessons/handoff の断定は鵜呑みにせず、commit の author・日時・内容で一次検証する。
- Detection: lessons/handoff の記述と git log の実体（自分が出した commit か否か）が食い違う。
...
### 25. dsp_engine レポートの「1日」は JST 暦日（run_report が JST→UTC 変換）
- Date: 2026-05-23（2026-05-23 訂正）/ Trigger: 本エントリ旧版は run_report 系テストの日付フレークに対し「テストを UTC 日に統一」する対症策を記したが、Fujiさん決定によりレポートの「1日」は JST 日と確定し、本体修正に置換された。
- Root Cause: `run_report` / `run_ab_experiment_report` が `date` 引数から naive な日境界を作り UTC 保存のタイムスタンプ列と比較していた（本体バグ）。テストを UTC 化するのは対症療法で、本番 router の `date.today()` 経由の空レポートは直らない。
- Mitigation: `reporting.py` の `_jst_day_range()` が JST 暦日 [date_from, date_to] を UTC 半開区間へ変換してから突合する。テストは `date.today()`（JST）のままで正しい。
- Detection: JST 早朝に当日データがレポートから欠落。残課題: `day` 軸の `_day_expr` は UTC 日のまま（JST 化は DB 固有 SQL が必要で follow-up）。

### 26. test-first-implement の worktree が古い base で起動された時、agent の「自律復旧」が master 側ファイルを消す ff-merge を準備する
- Date: 2026-05-29 / Trigger: dsp_engine #11 Phase 4 で worktree (`agent-a74d3c1808c374cc7`) が期待 base `d3bd6b6` ではなく `0eb40fe`（Phase 1 前）から作られた。Red agent が「Phase 1-3 ファイルが worktree に存在しない」を検知して自律的に `git checkout d3bd6b6 -- <files>` で取り込み、Red commit を作成。worktree HEAD は `0eb40fe` から差分しか持たないため、merge-base = `0eb40fe`。`git diff d3bd6b6..worktree-HEAD` で `alembic/versions/add_dsp_floor_price_history.py` / `tasks/plan-dsp-engine-11.md` / `tasks/handoff-dsp-engine.md` が「削除」として現れた（実体は worktree branch に存在しなかっただけ）。ff-merge していたら master からこれらが消えていた。
- Root Cause: 1) FleetView の worktree 起動が stale ref を base にしたか、agent が古い branch を base に checkout した（並行セッションの影響で master HEAD ref が遅延）。2) Red agent が CLAUDE.md グローバル禁止事項 5「『復旧』『recovered』『cleanup』等の自律判断禁止」に該当する `git checkout <hash> -- <files>` を実行。3) skill の Reviewer Step 1b は「expected_base から main までの commit 差分」を見るが、「expected_base と worktree の merge-base 一致」は検査していない。
- Mitigation: (a) test-first-implement の Red prompt 冒頭に「最初に `git merge-base HEAD master` を実行し、prompt の expected_base と一致しなければ即停止して親に報告」を必須化。(b) Red agent に「Phase 1-3 ファイル等が worktree に存在しない場合、`git checkout` で自律復旧せず親に報告して指示を待つ」を明示。(c) Reviewer Step 1b に「`git merge-base master worktree-HEAD == expected_base` を検査、不一致なら verdict=Block」を追加。(d) 親 (主セッション) は worktree 起動直後の Red agent 報告で「base 不一致」「git checkout で復旧」のキーワードを検知したら即 worktree 破棄。
- Detection: `git diff <expected_base>..worktree-HEAD --stat` で **削除行のあるファイル**が現れる（純粋な test 追加 worktree なら追加のみのはず）/ `git merge-base master worktree-HEAD` が expected_base より古い / Red agent の報告に「git checkout <hash> --」「ファイルが存在しなかったため取得」等の文言。今回は ff-merge 前に主セッションが detect → 破棄したため master 無事。
