...
**本番デプロイ状況**: Phase 2.5 まで本番反映済み。Phase 2.6 + 優先 #1〜#3 は master に push 済みだが
**Vercel 未デプロイ**（Git 未連携のため `vercel --prod` の手動実行が必要）。マイグレーション
dspengine0003・0004 は本番 Postgres 未適用。
...
### DBテーブル（マイグレーション dspengine0001〜0004）
...
- 全体: `pytest tests/` → **246 passed**, 6 failed, 1 skipped。
...
完了: #1・#2・#3。状態詳細・該当ファイルは `tasks/progress-dsp-engine.md` の優先タスク表参照。

| # | やること | 優先度 |
|---|---|---|
| 4 | 入札ログ完全化 + 予算 TOCTOU 対策（no-bid 理由コード `nbr` を含む）| **高（次着手）** |
| 5 | pCTR / pCVR / value / win-rate のベースライン ML（device 特徴量・WARM_THRESHOLD 設定化）| 中 |
| 6 | creative / publisher / app / placement / geo / device / deal_id 別レポート | 中 |
...
1. **#1〜#3 は push 済みだが本番未デプロイ**: `vercel --prod` 未実行。dspengine0003・0004
   マイグレーションも本番未適用。本番反映時は `vercel --prod`。
