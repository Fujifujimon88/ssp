...
async def create_slot(
    ...
    # sizes が指定されていれば最初のサイズを width/height に使う
    sizes = data.sizes or ([[ data.width or 300, data.height or 250 ]])
    primary_w, primary_h = sizes[0][0], sizes[0][1]
...
def _to_slot(slot: AdSlotDB) -> AdSlot:
    ...
    sizes = json.loads(slot.sizes) if slot.sizes else []
    return AdSlot(
        id=slot.id,
